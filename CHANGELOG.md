...
- **只读阅读器。** 除空格 Quick Look 预览外，现在可直接在独立窗口中打开并阅读全部 28 种文件类型，渲染效果与 Quick Look 完全一致。入口：Finder 右键「打开方式 → iM」、`open -a iM 文件`、应用内 ⌘O 与「最近打开」。严格只读，无任何编辑或保存操作。
- **实时刷新。** 文件被外部编辑器保存时窗口自动更新：增量替换正文、不重载页面、保留滚动位置；兼容 VS Code/vim 的原子 rename 保存；文件删除后重建可自动恢复。
- **窗口与进程生命周期。** 重复打开同一文件只前置已有窗口；关闭所有阅读窗口即退出；无文件启动（仅注册扩展）约 0.5 秒后自动退出，不驻留后台。

### 重构

- **统一渲染入口。** 抽出 `FileContentRenderer`，按扩展名分发的逻辑由 Quick Look 扩展与阅读器共用，保证两种打开方式的渲染结果始终一致。
